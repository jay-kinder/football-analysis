...
def create_df(file, event_type_id="None"):
    ...
    for game in file:
        for event in game:
            if event.attrib.get("type_id") == str(event_type_id) or event_type_id == "None":
    ...
    column_titles = ["team", "half", "min", "x_origin", "y_origin", "x_destination", "y_destination", "outcome", "event_type", "is_own_goal", "is_header", "is_freekick", "is_assist"]
    df = pd.DataFrame(data=[team, half, minute, x_origin, y_origin, x_destination, y_destination, outcome, event_type, is_own_goal, is_header, is_freekick, is_assist], index=column_titles)
    df = df.T
