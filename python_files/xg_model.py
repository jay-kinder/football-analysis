...
def xG_model(df):
    ...
    H_Shot = np.histogram2d(
        shots_model["y"], shots_model["x"], bins=50, range=[[0, 100], [0, 100]]
    )
    goals_only = shots_model[shots_model["goal"] == 1]
    H_Goal = np.histogram2d(
        goals_only["y"], goals_only["x"], bins=50, range=[[0, 100], [0, 100]]
    )
    ...
    ax.annotate(
        "Proportion of shots resulting in a goal", (100, 102), ha="left", fontsize=10
    )
...
def all_shots_by_type(df):
    ...
    for i, row in df.iterrows():
        ...
        if (
            row["event_type"] == "13"
            or row["event_type"] == "14"
            or row["event_type"] == "15"
        ):
            if row["is_header"] == "1":
                pitch.scatter(
                    marker_position[0],
    ...
    ax.annotate(
        "All Shots/Goals From All Reported Games (in Model)",
        (100, 102),
        ha="left",
        fontsize=10,
    )
    ...
    # save file
    fig.savefig(
        f"static/all_shots_from_model.png", bbox_inches="tight", pad_inches=0, dpi=200
    )
